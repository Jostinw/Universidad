...
import Controlador.busqueda.BusquedaLineal;
...
public class CensoController extends DataAccessObject<Censo> {
    ...
    public LinkedList<Censo> buscar(String elemento, String metodo, String atributo) {

        LinkedList<Censo> censos = listAll();

        if (metodo.equalsIgnoreCase("lineal")) {
            return new BusquedaLineal().busquedaBinaria(censos, atributo, elemento);
        } else {
            return new BusquedaBinaria().busquedaBinaria(censos, atributo, elemento);
        }
